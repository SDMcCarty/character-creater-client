...
function getRandomAge() {
  // odds of getting a certain age range to equalize the ages given a bit
  ...
  if (odds <= 1) {
    ...
    //ages 60 -123
    return Math.floor((Math.random() *  63) + 61)
  } else {
    //ages 15 - 59
    return Math.floor((Math.random() * 49) +15)
  }
}

function getRandomSex() {
  //better odds for male and female based on the real world percentages
  //should be adjusted as more information is gathered
  let odds = Math.floor((Math.random() * 7) + 1)
  if (odds === 7) {
    let sex = ['trans man', 'trans woman', 'intersex']
...
function getRandomMajorTrait() {
  //should add traits as they are created
